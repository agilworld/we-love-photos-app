...
export const useSearchOptionStore = create<SettingOptionProps>((set) => ({
    ...
    setColor:(value)=>set((state) => ({color:value as keyof typeof EColorProps})),
    setOrientation:(value)=>set((state) => ({orientation:value as keyof typeof EOrientationProps})),
}))
