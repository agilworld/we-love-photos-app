...
export default function PhotoGrid({keyword}:PhotoGridProps) {
    ...
    const { appendPhotos, clearPhotos, refreshPhotos } = usePhotoStore((state)=>state)
    ...
    return <>
        ...
        my-4 text-gray-500 text-sm">
            ...
            <div className="">
                ...
                <Badge variant={"secondary"}>
                    Page : {pagenumber} of {format.number(data?.total_pages??0)}</Badge> : 
                            data?.total === 0 ? null : 
       ...
       <PhotoGridMemoized 
        ...
        {isFetched && hasNextPage && <div className="flex 
...
function arePropsEqual(
    ...
    return (
        (newProps.isFetched && oldProps.isFetched === newProps.isFetched) && 
        (newProps.isSuccess && oldProps.isSuccess === newProps.isSuccess) &&
...
const PhotoGridMemoized = memo(function PhotoGridResult(
    { isFetching, isFetched, isSuccess, isMoredata, rows } : PhotoGridMemoizedProps
   ) {
    ...
    return (
        <div className="mt-10">
            {!isFetched && !isMoredata ? <PhotoGridLoader /> : rows.length > 0 ? 
                <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
                    {rows.map((items, chunkIdx) => <div 
                        key={chunkIdx} 
                        className="flex flex-col gap-5">
                        {items.map((item, idx)=><PhotoGridItem 
                            key={item.id} 
            ...
            <p className="text-center text-slate-700 
                ...
                border-gray-200 py-10">Not found data</p> : null}
